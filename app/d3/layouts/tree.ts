...
import 'app/d3/layouts/tree.scss!';

interface Node extends d3.layout.tree.Node {
	name?:string;
}

interface Link<Node> extends d3.layout.tree.Link<Node> {
}

interface Tree<Node> extends d3.layout.Tree<Node> {
}
...
export class TreeLayout implements OnInit {
	...
	ngOnInit() {
		this.http
			...
			.subscribe(r => {
				...
				// Layout
				//----------------------------------------------------------------
				let tree:Tree<Node> = d3.layout.tree<Node>()
				...
				// nodes(root: T): T[]
				let nodes:Node[] = tree.nodes(root);
				...
				// source → from
				// target → to
				let links:Link<Node>[] = tree.links(nodes);
